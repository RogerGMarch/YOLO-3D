...
class ObjectDetector:
    ...
    def detect(self, image, track=True):
        """
        Detect objects and keypoints in an image
        ...
        Returns:
            tuple: (annotated_image, detections)
                - annotated_image (numpy.ndarray): Image with detections and keypoints drawn
                - detections (list): List of detections [bbox, score, class_id, object_id, keypoints]
        """
        detections = []
        ...
        if track:
            ...
            for predictions in results:
                ...
                # Process boxes
                for i, bbox in enumerate(predictions.boxes):
                    # Extract information
                    scores = bbox.conf
                    classes = bbox.cls
                    ...
                    for j, (score, class_id, bbox_coord, id_) in enumerate(zip(scores, classes, bbox_coords, ids)):
                        xmin, ymin, xmax, ymax = bbox_coord.cpu().numpy()
                        
                        # Extract keypoints if available
                        keypoints = None
                        if hasattr(predictions, 'keypoints') and predictions.keypoints is not None:
                            # Get keypoints for this detection
                            kpts = predictions.keypoints[i].data[0].cpu().numpy()
                            
                            # kpts has shape (num_keypoints, 3) where each row is (x, y, confidence)
                            ...
                            # Draw skeleton connections (if desired)
                            # You'll need to add the skeleton connections based on the model's keypoint format
                        ...
                        detections.append([
                            ...
                            keypoints                  # keypoints [x, y, confidence] for each keypoint
                        ])
            ...
            for predictions in results:
                ...
                for i, bbox in enumerate(predictions.boxes):
                    ...
                    # Process each detection
                    for j, (score, class_id, bbox_coord) in enumerate(zip(scores, classes, bbox_coords)):
                        xmin, ymin, xmax, ymax = bbox_coord.cpu().numpy()
